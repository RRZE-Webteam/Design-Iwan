...
function iwan_entry_meta() {
	...
	if ( 'post' == get_post_type() ) {
		printf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
			...
			esc_attr( sprintf( __( 'View all posts by %s', 'iwan' ), get_the_author() ) ),
			get_the_author()
		);
	}
...
function iwan_entry_date( $echo = true ) {
	...
	$date = sprintf( '<span class="date"><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s%5$s%6$s</time></a></span>',
		esc_url( get_permalink() ),
		esc_attr( sprintf( __( 'Permalink to %s', 'iwan' ), the_title_attribute( 'echo=0' ) ) ),
		esc_attr( get_the_date( 'c' ) ),
		'<div class="entry-date-day">'.esc_html( sprintf( $format_prefix, get_post_format_string( get_post_format() ), get_the_date('d') ) ).'</div>',
		'<div class="entry-date-month-year"><span class="entry-date-month">'.esc_html( sprintf( $format_prefix, get_post_format_string( get_post_format() ), get_the_date('M') ) ).'</span><br>',
		'<span class="entry-date-year">'.esc_html( sprintf( $format_prefix, get_post_format_string( get_post_format() ), get_the_date('Y') ) ).'</span></div>'
	);
...
if (!function_exists('get_iwan_custom_excerpt')) :
	...
	function get_iwan_custom_excerpt($length = 100) {
		global $post;
		if (has_excerpt()) {
		...
		$the_str = '<p>' . $the_str;
		$the_str .= new_excerpt_more($more = 1);
		$the_str .= '</p>';
		return $the_str;
	}
