<?php
/**
 * Iwan functions and definitions
 *
...
/*
	 * Makes Iwan available for translation.
	 *
	 * Translations can be added to the /languages/ directory.
	 * If you're building a theme based on Iwan, use a find and
	 * replace to change 'iwan' to the name of your theme in all
	 * template files.
	 */
	load_theme_textdomain('iwan', get_template_directory() . '/languages');
 ...
 *
 * @see iwan_content_width() for template-specific adjustments.
 */
...
/**
 * Iwan only works in WordPress 3.6 or later.
 */
...
function iwan_setup() {
	...
	register_nav_menu('primary', __('Navigation Menu', 'iwan'));
...
function iwan_scripts_styles() {
	...
	// Loads JavaScript file with functionality specific to Iwan.
	wp_enqueue_script('iwan-script', get_template_directory_uri() . '/js/functions.js', array('jquery'), '2014-06-08', true);

	// Add Genericons font, used in the main stylesheet.
	wp_enqueue_style('genericons', get_template_directory_uri() . '/genericons/genericons.css', array(), '3.03');

	// Loads our main stylesheet.
	wp_enqueue_style('iwan-style', get_stylesheet_uri(), array(), '2013-07-18');

	// Loads the Internet Explorer specific stylesheet.
	wp_enqueue_style('iwan-ie', get_template_directory_uri() . '/css/ie.css', array('iwan-style'), '2013-07-18');
 ...
 */
function iwan_wp_title($title, $sep) {
	global $paged, $page;
	...
	if (( $paged >= 2 || $page >= 2 ) && !is_404())
		$title = "$title $sep " . sprintf(__('Page %s', 'iwan'), max($paged, $page));

	return $title;
}

add_filter('wp_title', 'iwan_wp_title', 10, 2);
 ...
 * @since Iwan 1.0
 */
function iwan_widgets_init() {
	register_sidebar(array(
		'name' => __('Main Widget Area', 'iwan'),
		'id' => 'sidebar-1',
		'description' => __('Appears in the footer section of the site.', 'iwan'),
	...
	register_sidebar(array(
		'name' => __('Secondary Widget Area', 'iwan'),
		'id' => 'sidebar-2',
		'description' => __('Appears on posts and pages in the sidebar.', 'iwan'),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
...
add_action('widgets_init', 'iwan_widgets_init');

if (!function_exists('iwan_paging_nav')) :
	...
	function iwan_paging_nav() {
		...
		<nav class="navigation paging-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php _e('Posts navigation', 'iwan'); ?></h1>
			<div class="nav-links">

				<?php if (get_next_posts_link()) : ?>
					<div class="nav-previous"><?php next_posts_link(__('<span class="meta-nav">&larr;</span> Older posts', 'iwan')); ?></div>
				<?php endif; ?>

				<?php if (get_previous_posts_link()) : ?>
					<div class="nav-next"><?php previous_posts_link(__('Newer posts <span class="meta-nav">&rarr;</span>', 'iwan')); ?></div>
				<?php endif; ?>
...
if (!function_exists('iwan_post_nav')) :
	...
	function iwan_post_nav() {
		...
		<nav class="navigation post-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php _e('Post navigation', 'iwan'); ?></h1>
			<div class="nav-links">

				<?php previous_post_link('%link', _x('<span class="meta-nav">&larr;</span> %title', 'Previous post link', 'iwan')); ?>
				<?php next_post_link('%link', _x('%title <span class="meta-nav">&rarr;</span>', 'Next post link', 'iwan')); ?>
...
if (!function_exists('iwan_entry_meta')) :

	/**
	 * Print HTML with meta information for current post: categories, tags, permalink, author, and date.
	 *
	 * Create your own iwan_entry_meta() to override in a child theme.
	 *
	 * @since Iwan 1.0
	 */
	function iwan_entry_meta() {
		...
		// Translators: used between list items, there is a space after the comma.
		$tag_list = get_the_tag_list('', __(', ', 'iwan'));
		if ($tag_list && !is_home()) {
			echo '<span class="tags-links">' . $tag_list . '</span>';
		}

		// Post author
		if ('post' == get_post_type() && ($options['aktiv-autoren'] == 1)) {
			printf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>', esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_attr(sprintf(__('View all posts by %s', 'iwan'), get_the_author())), get_the_author()
			);
		}
...
if (!function_exists('iwan_entry_date')) :

	/**
	 * Print HTML with date information for current post.
	 *
	 * Create your own iwan_entry_date() to override in a child theme.
	 *
	 * @since Iwan 1.0
	 *
	 ...
	 */
	function iwan_entry_date($echo = true) {
		if (has_post_format(array('chat', 'status')))
			$format_prefix = _x('%1$s on %2$s', '1: post format name. 2: date', 'iwan');
		else
			$format_prefix = '%2$s';

		$date = sprintf('<span class="date"><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s%5$s%6$s</time></a></span>', esc_url(get_permalink()), esc_attr(sprintf(__('Permalink to %s', 'iwan'), the_title_attribute('echo=0'))), esc_attr(get_the_date('c')), '<div class="entry-date-day">' . esc_html(sprintf($format_prefix, get_post_format_string(get_post_format()), get_the_date('d'))) . '</div>', '<div class="entry-date-month-year"><span class="entry-date-month">' . esc_html(sprintf($format_prefix, get_post_format_string(get_post_format()), get_the_date('M'))) . '</span><br>', '<span class="entry-date-year">' . esc_html(sprintf($format_prefix, get_post_format_string(get_post_format()), get_the_date('Y'))) . '</span></div>'
		);
...
if (!function_exists('iwan_the_attached_image')) :
	 ...
	 * @since Iwan 1.0
	 */
	function iwan_the_attached_image() {
		/**
		 ...
		 * }
		 */
		$attachment_size = apply_filters('iwan_attachment_size', array(724, 724));
 ...
 */
function iwan_body_class($classes) {
	if (!is_multi_author())
...
add_filter('body_class', 'iwan_body_class');
...
add_action('template_redirect', 'iwan_content_width');
 ...
 */
function iwan_customize_register($wp_customize) {
	$wp_customize->get_setting('blogname')->transport = 'postMessage';
	$wp_customize->get_setting('blogdescription')->transport = 'postMessage';
	$wp_customize->get_setting('header_textcolor')->transport = 'postMessage';
}

add_action('customize_register', 'iwan_customize_register');
 ...
 * @since Iwan 1.0
 */
function iwan_customize_preview_js() {
	wp_enqueue_script('iwan-customizer', get_template_directory_uri() . '/js/theme-customizer.js', array('customize-preview'), '20130226', true);
}

add_action('customize_preview_init', 'iwan_customize_preview_js');

// Replaces the excerpt "more" text by a link
function new_excerpt_more($more) {
	global $post;
	return '<br><a class="more-link" href="' . get_permalink($post->ID) . '">' . __('Read more &rarr;', 'iwan') . '</a>';
}
...
function iwan_breadcrumbs() {
	...
	echo '<span class="youarehere">' . __('You are here: ', 'iwan') . '</span>&nbsp;&nbsp;';
	...
	if (!is_home() && !is_front_page() || is_paged()) {
		...
		if (is_category()) {
			...
			echo $before . __('Category Archives ', 'iwan') . '"' . single_cat_title('', false) . '"' . $after;
		} elseif (is_day()) {
			...
			echo $before . __('Search for ', 'iwan') . '"' . get_search_query() . '"' . $after;
		} elseif (is_tag()) {
			echo $before . __('Tag Archives ', 'iwan') . '"' . single_tag_title('', false) . '"' . $after;
		} elseif (is_author()) {
			global $author;
			$userdata = get_userdata($author);
			echo $before . __('Articles by ', 'iwan') . $userdata->display_name . $after;
		} elseif (is_404()) {
		...
		/*
		  ...
		  echo __('Page', 'iwan') . ' ' . get_query_var('paged');
		  if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		  }
		 */
...
function iwan_excerpt_length($length) {
...
function iwan_auto_excerpt_more($more) {
	return ' &hellip;' . iwan_continue_reading_link();
}
add_filter('excerpt_more', 'iwan_auto_excerpt_more');

function iwan_custom_excerpt_more($output) {
	if (has_excerpt() && !is_attachment()) {
		$output .= iwan_continue_reading_link();
	}
	return $output;
}
add_filter('get_the_excerpt', 'iwan_custom_excerpt_more');


if (!function_exists('get_iwan_custom_excerpt')) :
	/*
	 * Erstellen des Extracts
	 */

	function get_iwan_custom_excerpt($length = 0) {
		global $options;
		global $post;
		...
		$the_str .= iwan_continue_reading_link();
		echo $the_str;
	}
